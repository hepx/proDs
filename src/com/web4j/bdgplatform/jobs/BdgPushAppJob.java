 ...
 *
 */
@Service("bdgPushAppJob")
public class BdgPushAppJob extends QuartzJobBean {

	private static Logger log = Logger.getLogger(BdgPushAppJob.class);
	
	private static final String JOBNAME="bdg_pushapp";
	private static final String JOBGROUP="job_group";
	private static final String TRIGGERNAME="bdg_pushapp_trigger";
	...
	protected void executeInternal(JobExecutionContext context)
		...
		try{
			logsModel.insertOneLog("执行BDG推送APP任务......");
			bdgAppPushModel.pushRecord();
		}catch(Exception e){
			//e.printStackTrace();
			String error="BDG定时推送APP任务异常。"+e.getMessage();
			try {
	...
	public void start(String cronExpression) throws Exception{
		...
		logs.insertOneLog("启动BDG定时推送APP任务...");
	...
	public void resume() throws Exception{
		logs.insertOneLog("恢复BDG定时推送APP任务...");
		QuartzScheduleMgr.resumeJob(JOBNAME, JOBGROUP);
	}
	
	public void pause() throws Exception{
		logs.insertOneLog("暂停BDG定时推送APP任务...");
	...
	public BdgAppPushModel getBdgAppPushModel() {
		return bdgAppPushModel;
	}

	public void setBdgAppPushModel(BdgAppPushModel bdgAppPushModel) {
		this.bdgAppPushModel = bdgAppPushModel;
	}
