...
public class Timeline extends Weibo{
	 ...
	 * 获得指定用户的微博列表
	 * @param uid	用户ID
	 * @param since_id	若指定此参数，则返回ID比since_id大的微博（
	 * @param feature	过滤类型ID，0：全部、1：原创、2：图片、3：视频、4：音乐
	 * @param limit 一次查询的记录数
	 * @return
	 * @throws WeiboException
	 */
	public StatusWapper getUserTimelineByUid(String uid,String since_id,int feature,int limit) throws WeiboException {
		return Status.constructWapperStatus(client.get(WeiboConfig
				.getValue("baseURL") + "statuses/user_timeline.json",new PostParameter[]{
			new PostParameter("uid", uid),new PostParameter("since_id", since_id),
			new PostParameter("feature", feature),new PostParameter("count", limit)
		}));
	}
	 ...
	 * @throws WeiboException
	 */
	public StatusWapper getUserTimelineByName(String screen_name,String since_id,int feature,int limit) throws WeiboException {
		return Status.constructWapperStatus(client.get(WeiboConfig
				.getValue("baseURL") + "statuses/user_timeline.json",new PostParameter[]{
			new PostParameter("screen_name", screen_name),new PostParameter("since_id", since_id),
			new PostParameter("feature", feature),new PostParameter("count", limit)
		}));
	}
